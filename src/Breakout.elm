...
checkBrick model brick =
  if model.ball.x >= brick.x - brickWidth/2
   ...
   && model.ball.y + ballRadius/2 >= brick.y - brickWidth/2
   && model.ball.y + ballRadius/2 <= brick.y + brickWidth/2 then
     { ball = { x = model.ball.x --vertical bounce
           , y = model.ball.y
           , dx = model.ball.dx
           , dy = -1 * model.ball.dy
           , collided = False
     ...
     , paddle = model.paddle
     , bricks = List.filter (\b -> b /= brick) model.bricks
     }
  else if model.ball.x + ballRadius/2 >= brick.x - brickWidth/2
   && model.ball.x + ballRadius/2 <= brick.x + brickWidth/2
   && model.ball.y >= brick.y - brickWidth/2
   && model.ball.y <= brick.y + brickWidth/2 then
     { ball = { x = model.ball.x --horizontal bounce
           , y = model.ball.y
           , dx = -1 * model.ball.dx
           , dy = model.ball.dy
           , collided = False
  ...
  else if model.ball.x >= brick.x - brickWidth/2
   && model.ball.x <= brick.x + brickWidth/2
   && model.ball.y - ballRadius/2 >= brick.y - brickWidth/2
   && model.ball.y - ballRadius/2 <= brick.y + brickWidth/2 then
     { ball = { x = model.ball.x --vertical bounce
...
brickCollision model =
  List.map (checkBrick model) model.bricks
